...
export default function SeatForm({ tables, onSubmit, onCancel, setFormData }) {
    ...
    return (
        <form onSubmit={onSubmit}>
                ...
                <select id="table_id" name="table_id" className="form-control bg-dark text-white" onChange={handleChange} required>
                    <option value>Select a Table</option>
                    {tableOptions}
                </select>
                <div className="mt-2">
                    <button type="button" className="btn btn-dark mr-2" onClick={onCancel}>Cancel</button>
